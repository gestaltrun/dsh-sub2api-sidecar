...
| `route.name` / `route.api` / `route.displayName` | `sub2api` / `openai-completions` / `Sub2API (sub2api)` | Provider identity. Models come only from account synchronization intersected with the group-bound gateway. |
...
intersection unregisters the provider, so neither a new installation nor an
operator configuration can invent a model the account pool cannot serve.
