...
Provider 模型目录取组绑定推理 key 的 `/v1/models` 与该 Composite 组账号已保存
`model_mapping` 标识的交集。每个可用条目会把网关公布的上下文与输出上限、文字／
图片输入模态、可选推理档位和逐模型推理默认值带入 `llm-pi-ai` profile；缺失或不受
支持的事实保持省略。没有已保存模型映射的账号不贡献模型，因此平台级网关目录不能
静默扩张 Provider。账号、分组与 Composite 路由管理请求成功后会立即请求刷新；
