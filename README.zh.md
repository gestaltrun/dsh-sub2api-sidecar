...
Provider 模型目录以组绑定推理 key 的 `/v1/models` 为准。每个可用实时条目会把其
公布的上下文与输出上限、文字／图片输入模态、可选推理档位和逐模型推理默认值带入
`llm-pi-ai` profile；缺失或不受支持的事实保持省略。账号、分组与 Composite 路由
管理请求成功后会立即请求刷新；`modelCatalogPollMs` 负责覆盖上游缓存失效和
旁路变更。实时请求失败时保留最近一次 Provider 设置；实时目录为空且未显式配置
`route.models` 时会注销 Provider，因此新安装不会虚构账号池无法提供的模型。显式
配置的路由模型只作为运维回退。
